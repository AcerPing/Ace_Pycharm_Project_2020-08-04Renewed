...
def frontpage(request):
    user = request.user #使用者
    record_form = RecordForm(user=user, initial={'balance_type':'支出'})
    records = Record.objects.filter(user = user) # 取得該使用者的某一範圍資料
...
def addCategory(request):
    if request.method == "POST":
        ...
        if category.strip() != '': Category.objects.get_or_create(category=category, user=user)
    return redirect("/settings")

@login_required # 必須要在使用者登入的情況下才能使用
def deleteCategory(request, category):
    user = request.user
    Category.objects.filter(category=category, user=user).delete() # 刪除一筆資料
    return redirect("/settings")
