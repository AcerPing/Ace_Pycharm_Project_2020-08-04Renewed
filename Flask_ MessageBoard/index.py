...
def change_message(modified_number, modified_name, modified_comment):
    ...
    # 執行插入方法，入的SQL語句
    # 格式化的作用是為了防止SQL注入，`的作用是為了防止關鍵詞被數據庫解析
    cur.execute("UPDATE `article` SET `name`=%s,`comment`=%s WHERE `id`=%s",
                [modified_name, modified_comment, modified_number])
    conn.commit()
    conn.close()
...
def load_data():
    ...
    # "SELECT * FROM `article` LIMIT 10 OFFSET 5"
    list = cur.fetchall()
    conn.commit()
    conn.close()
    return list
...
def load_data_page(page=1):
    ...
    try:
        ...
        cur.execute("SELECT * FROM `article` LIMIT 10 OFFSET 0")
        # cur.execute("SELECT * FROM `article`")
        # "SELECT * FROM `article` LIMIT 10 OFFSET 5"
    finally:
        list = cur.fetchall()
